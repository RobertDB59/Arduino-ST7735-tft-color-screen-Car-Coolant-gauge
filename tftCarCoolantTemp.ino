...
const unsigned char engineCoolant [] PROGMEM = {     // w =  24 h = 24
0x00, 0x18, 0x00, 0x00, 0x18, 0x00, 0x00, 0x18, 0x00, 0x00, 0x1F, 0xC0, 0x00, 0x1F, 0xC0, 0x00,
0x18, 0x00, 0x00, 0x18, 0x00, 0x00, 0x1F, 0xC0, 0x00, 0x1F, 0xC0, 0x00, 0x18, 0x00, 0x00, 0x18,
0x00, 0x00, 0x1F, 0xC0, 0x00, 0x1F, 0xC0, 0x00, 0x18, 0x00, 0x80, 0x99, 0x01, 0x41, 0x3C, 0x82,
0x36, 0x7E, 0x6C, 0x08, 0x3C, 0x10, 0x00, 0x18, 0x00, 0x80, 0x81, 0x01, 0x41, 0x42, 0x82, 0x36,
0x24, 0x6C, 0x08, 0x18, 0x10, 0x00, 0x00, 0x00, 
};
...
float ltx = 0.0;                            // Saved x coord of bottom of needle

int xca1       =   120;                     // x position center arc
int yca1       =   150;                     // y position center arc
int radx1      =    96;                     // x-axes radius
int rady1      =    96;                     // y-axes radius
...
void loop() {
  if (millis() - runTime >= 2000) {   // Execute every 0.2s
    ...
    reading = random( 0, 121);        // Test with random value
...
void showBackground() {
  ...
  tft1.fillRect   ( xDig1, yDig1 - 23, 67, 33, BLACK);       // draw background for 7 segment digits
  tft1.fillCircle ( xca1, yca1, 95, BLACK);
  tft1.fillRect   ( 28, yca1 + 1, 3, 9, BLACK);       // hide part of background

  tft1.drawBitmap(  10, 48, engineCoolant, 24, 24, LIGHTGREY);
  tft1.drawBitmap( 110,  4, renaultIcon  , 14, 17, DARKBLUE);

  for (int i = startScale; i < endScale; i += 2) {         // draw scale marks
    int tl = 12;                               // long scale mark length
    ...
    // BLUE zone limits temp < 50°C
    if (i >= -90 && i < -70) {
      tft1.fillTriangle ( x0, y0, x1, y1, x2, y2, CYAN);
      tft1.fillTriangle ( x1, y1, x2, y2, x3, y3, CYAN);
    }

    // GREEN zone limits 50 > temp < 90°C
    if (i >= -70 && i < -30) {
      tft1.fillTriangle ( x0, y0, x1, y1, x2, y2, GREEN);
      tft1.fillTriangle ( x1, y1, x2, y2, x3, y3, GREEN);
    }

    // RED zone limits temp > 90 TOO HIGH
    if (i >= -30 && i < 1) {
    ...
    // scale tick length
    if (i % 20 == 0) tl = 8;  // modulo 20
//    if (i % 4 != 0) tl = 4;  // modulo 10
...
//    }
    ...
    if (i % 10 == 0) {                         // put labels for scale
      x0 = sx * (radx1 + tl + 14) + xca1;     // calculate label positions
      y0 = sy * (rady1 + tl + 14) + yca1;
      tft1.setTextColor( LIGHTGREY);
      switch (i / 10) {
//        case -9: tft1.setCursor( x0 + 1, y0 + 2); tft1.print( "30"); break;  // position tweak
        case -7: tft1.setCursor( x0    , y0 - 1); tft1.print( "50"); break;  // position tweak
        case -3: tft1.setCursor( x0 - 3, y0 + 3); tft1.print( "90"); break;  // position tweak
      }
/*
    tft1.setTextColor( LIGHTGREY);
    tft1.setCursor ( 14, 98);
    tft1.setFont( &FreeSansBold9pt7b);
    tft1.print("RPM");
    tft1.setCursor ( 20, 116);
    tft1.setFont();
    tft1.print( "x 100");
*/
    }
    ...
    if (i < endScale - 1) tft1.drawLine ( x0, y0, x1, y1, WHITE); // draw bottom arc of scale
  }
...
void plotNeedle(int value, byte ms_delay) {
  ...
  if (value <  36) value =  36; // limit value to emulate needle end stops
  ...
  while (!(value == old_analog)) {
    ...
    float sdeg = map(old_analog, 30, 120, -180, -89); // Map value to angle 
    ...
    tft1.setTextColor( LIGHTGREY);
    tft1.setCursor ( 74, 28);
    tft1.setFont( &FreeSansBold9pt7b);
    tft1.print("C");
    tft1.setFont();

    ltx = tx;                         // store new needle end coords for next erase
    osx = sx * 92 + xca1;             // needle coords and needle length
    osy = sy * 92 + yca1;             // needle coords and needle length
